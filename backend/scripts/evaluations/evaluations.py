...
async def add_evaluations(db: AsyncSession) -> None:
    print("Seeding evaluations...")
    evaluations_file = current_dir / "evaluations.json"
    with Path.open(evaluations_file) as file:
        evaluations_data = json.load(file)

    evaluations_to_add = []
    for evaluation_data in evaluations_data:
        result = await db.execute(select(Evaluation).where(Evaluation.id == evaluation_data["id"]))

        if not result.scalars().first():
            if evaluation_data.get("date"):
                evaluation_data["date"] = datetime.fromisoformat(evaluation_data["date"])

            evaluations_to_add.append(Evaluation(**evaluation_data))
            print(f"  - Preparing to add Evaluations data {evaluation_data['id']}")
        else:
            print(f"  - Evaluations data {evaluation_data['id']} already exists, skipping.")
