...
async def get_units_by_class(
    db: Annotated[AsyncSession, Depends(get_db)], class_id: int
):
    return await unit_crud.get_units_by_class(db, class_id)
...
async def get_unit_items(
    ...
    if items is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Unit {id} not found"
        )
